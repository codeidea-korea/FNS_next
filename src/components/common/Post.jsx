    ...
    switch (postFrameTypeCd) {
      ...
      default:
        if (
          ...
          post?.post_images[0]?.post_video_yn === true
        ) {
          // 9:14 - 포스트가 단독 영상인 경우
          classNm = "rate_9vs14";
        } else {
        ...
        <>
          ...
          >
              ...
              if (item.post_video_yn === true) {
                return (
                  <SwiperSlide key={index} className={"video_item"}>
                    ...
                    >
                      <source
                        src={item.post_video_urls[0].video_file_path}
                        type="video/mp4"
                      />
                    </video>
                    ...
                    <div className="video_volume">
                      <button onClick={toggleMute}>
                        <img
                          src={
                            isMuted ? "/img/volume.svg" : "/img/volume_on.svg"
                          }
                ...
                return (
                  <SwiperSlide key={index}>
                    <img
                      ...
                      alt={`${post.post_desc} 이미지${index + 1}`}
                    />
                  </SwiperSlide>
                );
              }
            ...
            <div className="tag_list">
              {post.post_images.map((item, index) => {
                ...
                const imageAccImgSrc =
                  postImageAccFirst?.post_image_acc_tag[0]?.image_url1 ?? "";
                const imageAccName =
                  postImageAccFirst?.post_image_acc_tag[0]?.tag_name ?? "";
                ...
                return (
                  <span
                    ...
                    onClick={(e) =>
                      modalOpen(e, imageAccId, imageAccName, imageAccImgSrc)
                    }
                  >
                    {imageAccId}
                  </span>
            ...
            {/* 이미지 우측 상단의 번호 */}
            <div className="pager">
              {swiperActive + 1} / {post.post_images.length}{" "}
            </div>
          </Swiper>
          ...
          <div className="txt_box">
            ...
            <div className="top_btn">
              <div>
                <button onClick={openAppDownModalFn}>
            ...
            <div className="tag_box">
              {post.post_tags.map((item, index) => (
                <button key={index} onClick={openAppDownModalFn}>
                  {item.tag_name}
                </button>
              ))}
            </div>
              ...
              <div className="comment_list">
                ...
                {post.post_comment.slice(0, 2).map((item, index) => (
                  <div key={index} className="comment_item">
                    <b>{item.created_user_name}</b>
                    <span>{item.cmt_desc}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="comment_box">
              <i>
                <img src="/img/profile_64x64.svg" alt="" />
              </i>
              <div className="comment" onClick={openAppDownModalFn}>
                댓글 달기
              </div>
