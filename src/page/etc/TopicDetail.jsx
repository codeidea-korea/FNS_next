      ...
      if (swiper.clientWidth < swiperSize) {
        ...
        if (changePosition < 0) {
          if (changePosition < maxSwiper * -1) {
            // 마지막으로 고정
            swiperWrap.style.transform = `translate3d(${
              maxSwiper * -1 - 15
            }px, 0, 0)`;
          } else {
            // 가운데로 이동
              ...
              <div className={`topic_thumbnail`}>
                <a style={{ cursor: "pointer" }} onClick={openAppDownModal}>
                  ...
                  <div className="txt_box">
                    <h5
                      dangerouslySetInnerHTML={{ __html: data.vw_title }}
                    ></h5>
                    <p>{data.vw_desc}</p>
                  </div>
                </a>
                ...
                {filters.map((filter, filterIdx) => {
                  if (filter.vw_flt_use_yn === true) {
                    ...
                    return (
                      <SwiperSlide
                        key={filterIdx}
                        className={filterIdx === 0 && "active"}
                        onClick={cateClick}
                        data-target={`category_0${tempIdx2}`}
                      >
                        <span>{filter.vw_flt_name}</span>
                      </SwiperSlide>
                    );
                  }
                })}
              ...
              return (
                <React.Fragment key={vwGroupIdx}>
                  <section style={{ padding: "30px 20px 0" }}>
                    <h3 style={{ paddingBottom: "0" }} className="main_tit">
                      {group.grp_name}
                    </h3>
                  </section>
